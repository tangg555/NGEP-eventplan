...
from src.utils.model_utils import count_trainable_parameters
...
class Seq2SeqLoggingCallback(pl.Callback):
    ...
    def on_train_start(self, trainer, pl_module):
        ...
        params_stat = {"n_params": nparams, "mp": nparams / 1e6, "grad_mp": n_trainable_pars / 1e6}
...
class Seq2SeqCheckpointCallback(pl.callbacks.ModelCheckpoint):
    ...
    def __init__(self, output_dir, experiment_name, monitor="val_loss",
                 save_top_k=1, every_n_train_steps=1000, save_on_train_epoch_end=True,
        ...
        if self.save_on_train_epoch_end:
            super().__init__(dirpath=self.output_dir,
...
class EarlyStoppingCallback(pl.callbacks.EarlyStopping):
    def __init__(self, metric, patience, **kwargs):
        self.metric = metric
        self.patience = patience
        super(EarlyStoppingCallback, self).__init__(monitor=f"val_{self.metric}",  # does this need avg?
